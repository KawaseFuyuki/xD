...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'cash' || command === 'balance') {
    ...
    return message.reply(
      `<:owo_cash:1515357631466705039> | **${target.user.username}**, you currently have **${bal.toLocaleString()}** cowoncy!`
    );
  }
  ...
  if (command === 'prefix') {
    ...
    // Exact same text as OwO bot shown in screenshot
    return message.reply(
  ...
  if (command === 'give') {
    ...
    // - No bold on usernames in description
    // - ⚠️ warning text (not bold except "banned")
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.customId === 'cancel_give') {
    ...
    return interaction.followUp(
      `❌ | **${tx.senderName}** cancelled the cowoncy transfer to **${tx.targetName}**.`
    );
  }
});
